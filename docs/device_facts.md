...
| Watchdog/fault record | `hal_wdt` owns WWDT0 with an approximate 1-second watchdog period and main-loop servicing. The 32-byte `lib_crash` format records reset cause, exception reason, stacked PC/xPSR, sequence, and CRC in `.noinit`; it is host-tested and safely rejected when invalid. The WWDT reset cause and a deliberate HardFault record are bench-proven. | Power-cycle retention and watchdog normal-operation timing. |
| LaunchPad UART | The XDS110 backchannel routes target UART0 TX to PB6 (`PINCM17`, function `UART0_TX`) and UART0 RX to PB7 (`PINCM18`, function `UART0_RX`). Phase 1 configures TX only at 115200 baud from BUSCLK with 16x oversampling; RX remains disabled. | EVM target schematic, TI SDK C1106 UART example, and TI device header. The debug boot-banner terminal test is recorded above; burst and overflow evidence remain pending. |
